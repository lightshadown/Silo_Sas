...
bool Login::checkConfigFile(QString Dir){  // pasa la direccion de la aplicacion
    ...
    */
    QString path = Dir + "\\Data" ;  // Returns current program location + search folder
    QStringList foundFiles, file;
    file.append("config.db");
    ...
    if(foundFiles.isEmpty()){
        silo.Log("La busqueda no regreso ningun archivo: " + file.at(0));// QString::number(foundFiles.size()));
        fileConfig = false;
    }
    if (!foundFiles.isEmpty()){
        foundFiles.sort();
        if (foundFiles.at(0) == "config.db" ){
    ...
    // fileHandle = FindFirstFile(findDb,&archivo); // switch to checar db
