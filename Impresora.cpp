...
void Impresora::imprimir_RegInd( QWidget *parent, const std::vector<Data> &print ){  // este es el bueno
    Gen silo;
    QPrinter *printer;
    QPrintDialog dialog(parent);
...
void Impresora::Imp_Boleta(QPainter &painter, const std::vector<Data> &Dinamic_Data ){   // revisar   https://stackoverflow.com/questions/36249645/qt-reusable-paint-functions-with-custom-arguments
...
//    const QRect rectangle = QRect(0, 0, 850, 1000);  // rectangulo general donde va todo
    ...
    painter.drawText(QRect(780 - UbicX,588,40,20), 0, "Tons", &boundingRect);   // deduc
...
void Impresora::Imp_RegIndiv_Boletas(QPainter &painter, const std::vector<Data> &boletas, int NoPage) { 
    // revisar como imprimir varias hojas
    // https://stackoverflow.com/questions/43526087/how-to-print-multiple-qwidgets-to-a-pdf-in-different-pages-using-qpainter
    
    auto Total_Bruto{0.0}, Total_Tara{0.0}, Total_Neto{0.0}, Total_Analizado{0.0}, Total_Totales{0.0}; 
    Gen silo;
    std::time_t now_c;
    ...
    painter.drawText(QRect(550,200,500,20), 0, boletas[0].Comprador, &boundingRect);              // Comprador
    ...
    painter.drawText(QRect(220,305,180,20), 0, "1,000,000.00", &boundingRect);                    // diferencia de precio
    ...
    if( NoPage >= ((int)boletas.size()) /15) {
        ...
        painter.drawText(QRect(170,820,180,20), 0, "1,000,000.00", &boundingRect);                  // Secado
        painter.drawText(QRect(170,840,250,20), 0, "100,000,000.00", &boundingRect);                   // Cuota Grano
        painter.drawText(QRect(170,860,250,20), 0, boletas[0].Sanidad, &boundingRect);            // Sanidad
        painter.drawText(QRect(700,820,180,20), 0, "100,500,000.00", &boundingRect);                // SubTotal
        ...
        painter.drawText(QRect(700,780,200,20), 0, QString::number(Total_Totales, 'f', 3), &boundingRect);     // Total total
    }
